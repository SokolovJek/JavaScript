...
function clickHandler(event) {
    ...
    let el = event.target;
    let elInner = el.getAttribute('data-type');
    showCategory(elInner);
}
...
function showCategory(category) {
    ...
    let myDivProduct = getProductMarkup(category);
    productEl.innerHTML = myDivProduct;
...
function getProductMarkup(product) {
    let el = products[product];
    let productEl = '';
    el.forEach(function (el1) {
        productEl += `<div class="product">
            <div>${el1.name}</div>
            <img src="${el1.imageUrl}" alt="">
            <div>${el1.price}</div>
            <a href="producs/${el1.id}.html">Подробнее</a>
            </div>`;
    });
    return productEl;
